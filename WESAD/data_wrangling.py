...
def get_window_stats(data, label=-1):
    """
    Compute per-signal summary stats for a single window.

    Returns a dataframe indexed by signal name with columns:
    ...
    # Don't treat the original label stream as a sensor channel
...
def compute_features(e4_data_dict, labels, norm_type=None):
    ...
    # pandas>=3 removed fillna(method=...); use bfill/ffill instead
    df['label'] = df['label'].bfill().ffill()
...
def get_samples(data, n_windows, label):
    ...
    for i in range(n_windows):
        ...
        x = get_window_stats(data=w, label=label)
        y = label
...
def combine_files(subjects):
    ...
    # Reconstruct multiclass label from one-hot columns.
    # The per-subject CSVs store these as booleans (True/False), so string parsing fails.
    label_cols = [c for c in ['0', '1', '2'] if c in df.columns]
    df['label'] = df[label_cols].astype(int).idxmax(axis=1).astype(int)
